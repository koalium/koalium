...
// Switch pins
#define WINDOW1_PIN 3
#define WINDOW2_PIN 4
#define WINDOW3_PIN 5
#define DOOR_PIN 6
...
// Situation states
enum  Situation { NO_RISK, LOW_RISK, HIGH_RISK } ;
Situation currentSituation = NO_RISK;
...
void setup() {
  ...
  digitalWrite(AJIR_PIN, LOW_RISK);
...
void checkSwitches() {
  ...
  if (newWindow1State != window1State || newWindow2State != window2State || newWindow3State != window3State || newDoorState != doorState) {
    ...
    // Check if situation is HIGH_RISK and a window/door is opened
    if (currentSituation == HIGH_RISK && (window1State || window2State || window3State || doorState)) {
      wtfState = true;
...
void handleControl() {
  ...
  html += "<option value='0'" + String(currentSituation == NO_RISK ? " selected" : "") + ">NO_RISK</option>";
  html += "<option value='1'" + String(currentSituation == LOW_RISK ? " selected" : "") + ">LOW_RISK</option>";
  html += "<option value='2'" + String(currentSituation == HIGH_RISK ? " selected" : "") + ">HIGH_RISK</option>";
