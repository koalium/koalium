...
This code receives data from the first client, logs switch state changes to the SD card, and sends a command to the second client to turn on/off the relays.
...
void loop() {
  ...
  if (radio.available()) {
    ...
    if (data.door1State == LOW && data.door2State == LOW) { // Example condition for WTF=ON
      ...
      command.turnOnRelays = true;
      radio.stopListening();
      radio.write(&command, sizeof(command));
      radio.startListening();
      Serial.println("Sent command to turn ON relays.");
    } else {
      // Send a command to the second client to turn off the relays
      CommandPacket command;
      command.turnOnRelays = false;
      radio.stopListening();
      radio.write(&command, sizeof(command));
      radio.startListening();
      Serial.println("Sent command to turn OFF relays.");
    }
  }
}
